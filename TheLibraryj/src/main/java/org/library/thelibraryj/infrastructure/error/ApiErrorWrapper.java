    ...
    private static ApiErrorResponse generateWrapper(GeneralError error) {
        return switch (error) {
            ...
            case UserInfoError.UsernameUpdateCooldown e ->
                    getErrorResponseAndLog(error, HttpStatus.BAD_REQUEST, "Username update cooldown has not yet finished. Time left (days): " + e.cooldownDurationLeft() + ". For email: " + e.userEmail());

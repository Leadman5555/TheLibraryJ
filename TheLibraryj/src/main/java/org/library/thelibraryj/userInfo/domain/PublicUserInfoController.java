...
class PublicUserInfoController implements ErrorHandling {
    ...
    @Operation(
            summary = "Get top rated users. The ranking refreshes at certain times",
            tags = {"user", "no auth required"}
    )
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200",
                    description = "Successfully retrieved the top users information",
                    content = @Content(
                            mediaType = MediaType.APPLICATION_JSON_VALUE,
                            array = @ArraySchema(schema = @Schema(implementation = UserTopRankerResponse.class))
                    )
            )
    })
    @GetMapping("/top")
    public ResponseEntity<List<UserTopRankerResponse>> getTopRankingUsers() {
        return ResponseEntity.ok(userInfoService.getTopUsers());
    }
}

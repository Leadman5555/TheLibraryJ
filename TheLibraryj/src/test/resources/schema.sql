...
CREATE TABLE library.library_user_info(id         UUID   NOT NULL,    version    BIGINT NOT NULL DEFAULT 0,created_at TIMESTAMP,updated_at TIMESTAMP,data_updated_at TIMESTAMP, username   VARCHAR(25) UNIQUE NOT NULL, email      VARCHAR(50) UNIQUE NOT NULL, rank       TINYINT NOT NULL DEFAULT 0, current_score   INTEGER            NOT NULL DEFAULT 0, user_auth_id UUID NOT NULL, status          VARCHAR(300), preference      TINYINT           NOT NULL DEFAULT 0, CONSTRAINT pk_library_user_info PRIMARY KEY (id));
...
ALTER TABLE library.subscribed_books ADD CONSTRAINT fk_subscribed_books_on_user_info FOREIGN KEY (user_info_email) REFERENCES library.library_user_info (email) ON DELETE CASCADE;
...
CREATE TABLE  library.library_user_auth(id           UUID               NOT NULL,   version      BIGINT             NOT NULL DEFAULT 0,created_at   TIMESTAMP,updated_at   TIMESTAMP,password VARCHAR(200) NOT NULL,email        VARCHAR(50) UNIQUE NOT NULL,role         VARCHAR(12)      NOT NULL,is_enabled BOOLEAN NOT NULL DEFAULT false, is_google  BOOLEAN            NOT NULL,CONSTRAINT pk_library_user_auth PRIMARY KEY (id));
...
DROP TABLE IF EXISTS library.library_auth_tokens;
CREATE TABLE library.library_book_tokens(id          UUID      NOT NULL,token       UUID      NOT NULL,for_user_id UUID      NOT NULL,version     BIGINT    NOT NULL DEFAULT 0,created_at  TIMESTAMP,updated_at  TIMESTAMP,expires_at  TIMESTAMP NOT NULL,use_count    TINYINT   NOT NULL DEFAULT 0,CONSTRAINT pk_library_book_tokens PRIMARY KEY (id));

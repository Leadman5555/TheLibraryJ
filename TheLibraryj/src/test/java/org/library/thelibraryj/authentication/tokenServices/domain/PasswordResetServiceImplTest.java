...
class PasswordResetServiceImplTest {
    ...
    void testConsumePasswordResetTokenSuccess() {
        ...
        when(tokenRepository.findByToken(eq(tokenId))).thenReturn(Optional.of(token));
        when(passwordEncoder.encode(eq(new String(request.newPassword())))).thenReturn("encryptedPassword");
        when(userAuthService.updatePassword(eq(token.getForUserId()), eq("encryptedPassword".toCharArray())))
        ...
        verify(tokenRepository, times(1)).update(eq(token));
        verify(userAuthService, times(1)).updatePassword(eq(token.getForUserId()), eq("encryptedPassword".toCharArray()));
    }
    ...
    void testConsumePasswordResetTokenFailTokenNotFound() {
        ...
        when(tokenRepository.findByToken(eq(tokenId))).thenReturn(Optional.empty());
